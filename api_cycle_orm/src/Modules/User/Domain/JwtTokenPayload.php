...
class JwtTokenPayload implements JsonSerializable
{
    ...
    public static function fromTokenData(\stdClass $rawPayload = null): ?static
    {
        ...
        if($rawPayload && property_exists($rawPayload, 'iss')) {
            ...
            $issuedAt = property_exists($rawPayload,'iat') ? DateTimeHelper::createFromTimeStamp((int) $rawPayload->iat) : DateTimeHelper::createDateTime();
            $expiresAt = property_exists($rawPayload,'exp') ? DateTimeHelper::createFromTimeStamp((int) $rawPayload->exp) : DateTimeHelper::createDateTime($ttl);
